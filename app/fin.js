...
(function(){
  ...
  function formCredits(cfg){
    ...
    var rows = list.length? list.map(function(o){
      ...
      return '<div class="card" data-cr="'+o.id+'" style="margin-bottom:10px;padding:11px 14px;cursor:pointer">'+
        ...
        (closed?'':'<button class="btn" data-pay="'+o.id+'" style="background:var(--green);color:#04140b;margin-top:8px">Погасить</button><button class="btn btn-ghost" data-early="'+o.id+'" style="margin-top:6px">🏁 Закрыть досрочно</button>')+
      '</div>';
  ...
  function catBars(ops){ var by={},tot=0; ops.forEach(function(o){var k=o.category||o.project||'—';by[k]=(by[k]||0)+o.amount;tot+=o.amount;});
  ...
  if(CLOUD){
    ...
    var dn=document.querySelector('.demo-note'); if(dn)dn.style.display='none';
    ...
    window.API.finGet().then(function(res){
      var sd=res&&res.data;
      if(sd&&Array.isArray(sd.ops)&&sd.ops.length){ DB=sd; localStorage.setItem(KEY,JSON.stringify(DB)); render(); }
      else { doPush(); } // сервер пуст — первая миграция: заливаем локальные данные
    }).catch(function(){});
  } else {
    setRole('ruslan');
  }
